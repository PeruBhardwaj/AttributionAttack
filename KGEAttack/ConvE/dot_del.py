...
def get_deletions(train_data, test_data, neighbours, model, attack_batch_size):
    ...
    for test_idx, test_trip in enumerate(test_data):
        ...
        while b_begin < nghbr_trip.shape[0]:
            ...
            b_dot = torch.matmul(b_nghbr_vec, test_vec.t()) 
            b_dot = b_dot.detach().cpu().numpy().tolist()
            nghbr_dot += b_dot
            b_begin += nghbr_batch    
...
if __name__ == '__main__':
    ...
    log_path = 'logs/attack_logs/dot_del_{0}_{1}_{2}_{3}_{4}'.format( args.model, args.data, 
    ...
    save_path = 'data/dot_del_{0}_{1}_{2}_{3}_{4}'.format( args.model, args.data, 
                                                               args.target_split, args.budget, args.rand_run)
