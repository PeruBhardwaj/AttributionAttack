...
def get_deletions(train_data, test_data, neighbours, model, attack_batch_size):
    ...
    for test_idx, test_trip in enumerate(test_data):
        ...
        for train_trip in nghbr_trip:
            ...
            #dist = -torch.cdist(test_grads[None,:], train_grads[None,:]).squeeze()
            dist = -torch.norm((test_grads-train_grads), p=2, dim=-1)
            #dist = pdist(test_grads[None,:], train_grads[None,:]).squeeze()
...
if __name__ == '__main__':
    ...
    log_path = 'logs/attack_logs/l2_grad_del_{0}_{1}_{2}_{3}_{4}'.format( args.model, args.data, 
    ...
    save_path = 'data/l2_grad_del_{0}_{1}_{2}_{3}_{4}'.format( args.model, args.data, 
                                                               args.target_split, args.budget, args.rand_run)
