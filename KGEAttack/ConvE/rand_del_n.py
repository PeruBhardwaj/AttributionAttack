...
# This is the notebook for randomly deleting a triple from the neighbourhood of each target triple. Neighbourhood refers to the triples that share subject or object with the target triple
...
def get_deletions(test_data, train_data, rng):
    ...
    for test_idx, test_trip in enumerate(test_data):
        ...
        rnd_idx = rng.choice(a=test_nghbrs.shape[0], size = 1, replace=False)[0]
        #select a random index in neighbourhood
        trip_idx = test_nghbrs[rnd_idx] #this gives the training set index to be deleted
        rnd_trip = train_data[trip_idx]
...
if __name__ == '__main__':
    ...
    log_path = 'logs/attack_logs/rand_del_n_{0}_{1}_{2}_{3}_{4}.log'.format( args.model, args.data, args.target_split, args.budget, args.rand_run)
    ...
    save_path = 'data/rand_del_n_{0}_{1}_{2}_{3}_{4}'.format( args.model, args.data, 
