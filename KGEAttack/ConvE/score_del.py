...
# In this notebook, I delete a triple from the neighbourhood of the target triple based on the **difference of scores** between the candidate triple and the target triple
# 
...
def get_deletions(train_data, test_data, neighbours, model, attack_batch_size):
    ...
    for test_idx, test_trip in enumerate(test_data):
        ...
        test_score = model.score_triples(test_s, test_r, test_o)
        ...
        while b_begin < nghbr_trip.shape[0]:
            ...
            #nghbr_vec = model.score_triples_vec(nghbr_s, nghbr_r, nghbr_o)
            b_nghbr_score = model.score_triples(b_nghbr_s, b_nghbr_r, b_nghbr_o)
            #print(b_nghbr_score.shape)
            b_nghbr_score = b_nghbr_score.detach().cpu().numpy().tolist()
            nghbr_score += b_nghbr_score
            b_begin += nghbr_batch

        nghbr_score = np.array(nghbr_score)
        nghbr_score = torch.from_numpy(nghbr_score).to(device)

        score_diff = torch.abs(nghbr_score - test_score)

        # we want to remove the neighbour with minimum difference in scores
        max_values, argsort = torch.sort(score_diff, -1, descending=False)
        #max_values, argsort = torch.sort(cos_sim, -1, descending=True)
        del_idx = argsort[0]
...
log_path = 'logs/attack_logs/score_del_{0}_{1}_{2}_{3}_{4}.log'.format( args.model, args.data, 
...
save_path = 'data/score_del_{0}_{1}_{2}_{3}_{4}'.format( args.model, args.data, 
                                                           args.target_split, args.budget, args.rand_run)
