...
def get_decoy_o_addition(triple, model, train_data):
    '''
    Additions for decoy triples on object side, 
    i.e. [s,r,o'] is the decoy triple
    o' is selected as the entity that has minimum soft truth value (o',r,s)->(s,r,o')
    '''
    ...
    target_score = model.forward(emb_s, emb_r, mode='rhs', sigmoid=True).squeeze()
    target_truth = target_score - 1 # this is I(s,r,o') - 1

    # compute candidate scores for (o',r,s)
    candidate_truth = model.forward(emb_s,emb_r,mode='lhs',sigmoid=True).squeeze()
    ...
    min_values_o, argsort_o = torch.sort(ground_truth, -1, descending=False)
    max_val_o = -min_values_o[0].item()
    
    s,r,o = s.item(), r.item(), o.item()
    o_dash = argsort_o[0].item()
    decoy_o = [s,r,o_dash]
    adv_o = [o_dash, r, s]
    
    return max_val_o, adv_o, decoy_o

def get_decoy_s_addition(triple, model, train_data):
    '''
    Additions for decoy triples on the subject side,
    i.e. [s',r,o] is the decoy triple
    s' is selected as the entity that has minimum soft truth value (o,r,s')->(s',r,o)
    '''
    triple = torch.from_numpy(triple).to(device)[None,:]
    s,r,o = triple[:,0], triple[:,1], triple[:,2]
    ...
    emb_r = model.emb_rel(r).squeeze(dim=1)
    target_score = model.forward(emb_o, emb_r, mode='lhs',sigmoid=True).squeeze()
    target_truth = target_score - 1 # this is I(s',r,o) - 1

    # compute candidate scores for (o,r,s')
    candidate_truth = model.forward(emb_o, emb_r, mode='rhs',sigmoid=True).squeeze()
    ground_truth = candidate_truth*target_truth
    ground_truth += 1
    ...
    min_values_s, argsort_s = torch.sort(ground_truth, -1, descending=False)
    max_val_s = -min_values_s[0].item()
    
    s,r,o = s.item(), r.item(), o.item()
    s_dash = argsort_s[0].item()
    decoy_s = [s_dash, r, o]
    adv_s = [o, r, s_dash]
    
    return max_val_s, adv_s, decoy_s
...
def get_additions(train_data, test_data, model):
    ...
    for test_idx, test_trip in enumerate(test_data):
        ...
        max_val_s, add_trip_s, decoy_s = get_decoy_s_addition(test_trip, model, train_data)
        max_val_o, add_trip_o, decoy_o = get_decoy_o_addition(test_trip, model, train_data)
    ...
    return triples_to_add, decoy_triples, summary_dict  
...
if __name__ == '__main__':
    ...
    log_path = 'logs/attack_logs/sym_add_1_{0}_{1}_{2}_{3}_{4}'.format( args.model, args.data, 
    ...
    logger.info('-------------------- Edits with Symmetric Attack - soft truth ----------------------')
    ...
    triples_to_add, decoy_triples, summary_dict = get_additions(train_data, test_data, model)
    ...
    save_path = 'data/sym_add_1_{0}_{1}_{2}_{3}_{4}'.format( args.model, args.data, 
                                                               args.target_split, args.budget, args.rand_run)
    ...
    with open(os.path.join(save_path, 'stats.txt'), 'w') as f:
        ...
        f.write('----------------------Symmetric Add 1 (Soft Truth Score)------------------------------- \n')


    with open(os.path.join(save_path, 'additions.txt'), 'w') as out: ### note the difference from IJCAI_add
