...
def get_decoy_o_addition(triple, model, train_data):
    ...
    o' is selected as the entity that is farthest from o in embedding space by cosine distance
    '''
    ...
    max_val_o = 1.0 - min_values_o[0].item()  # cos_dist = 1-cos_sim
...
def get_decoy_s_addition(triple, model, train_data):
    ...
    i.e. [s',r,o] is the decoy triple
    s' is selected as the entity that is farthest from s in embedding space by cosine distance
    '''
    triple = torch.from_numpy(triple).to(device)[None,:]
    s,r,o = triple[:,0], triple[:,1], triple[:,2]
    ...
    max_val_s = 1.0 - min_values_s[0].item() # cos_dist = 1-cos_sim
...
if __name__ == '__main__':
    ...
    log_path = 'logs/attack_logs/sym_add_3_{0}_{1}_{2}_{3}_{4}'.format( args.model, args.data, 
    ...
    logger.info('-------------------- Edits with Symmetric Attack - cosine ----------------------')
    ...
    save_path = 'data/sym_add_3_{0}_{1}_{2}_{3}_{4}'.format( args.model, args.data, 
                                                               args.target_split, args.budget, args.rand_run)
    ...
    with open(os.path.join(save_path, 'stats.txt'), 'w') as f:
        ...
        f.write('----------------------Symmetric Add 3 (Cosine)------------------------------- \n')
