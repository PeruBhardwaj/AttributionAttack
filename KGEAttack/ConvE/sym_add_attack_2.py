...
def get_decoy_s_addition(triple, model, train_data, add_rec):
    '''
...
def get_additions(train_data, test_data, model, n_rel, args):
    ...
    for test_idx, test_trip in enumerate(test_data):
        ...
        max_val_s, add_trip_s, decoy_s = get_decoy_s_addition(test_trip, model, train_data, add_rec)
...
if __name__ == '__main__':
    ...
    log_path = 'logs/attack_logs/sym_add_2_{0}_{1}_{2}_{3}_{4}'.format( args.model, args.data, 
    ...
    logger.info('-------------------- Edits with Symmetric Attack - worse ranks ----------------------')
    ...
    triples_to_add, decoy_triples, summary_dict = get_additions(train_data, test_data, model, n_rel, args)
    ...
    save_path = 'data/sym_add_2_{0}_{1}_{2}_{3}_{4}'.format( args.model, args.data, 
                                                               args.target_split, args.budget, args.rand_run)
    ...
    with open(os.path.join(save_path, 'stats.txt'), 'w') as f:
        ...
        f.write('----------------------Symmetric Add 2 (Worse Ranks)------------------------------- \n')
