...
def get_decoy_o_addition(triple, model, train_data, inverse_relation):
    ...
    emb_rinv = model.emb_rel(r_inv).squeeze(dim=1)
    ...
    candidate_truth = model.forward(emb_s,emb_rinv,mode='lhs',sigmoid=True).squeeze()
...
def get_decoy_s_addition(triple, model, train_data, inverse_relation):
    '''
    ...
    # compute candidate scores for (o,ri,s')
    candidate_truth = model.forward(emb_o, emb_rinv, mode='rhs',sigmoid=True).squeeze()
    ground_truth = candidate_truth*target_truth
    ground_truth += 1
...
def get_additions(train_data, test_data, model):
    ...
    #     
    # 2. For every test triple, choose o' with min ground truth on o-side and choose s' with max cos distance on s-side
    ...
    for test_idx, test_trip in enumerate(test_data):
        ...
        max_val_s, add_trip_s, decoy_s = get_decoy_s_addition(test_trip, model, train_data, inverse_relation)
...
if __name__ == '__main__':
    ...
    log_path = 'logs/attack_logs/inv_add_1_{0}_{1}_{2}_{3}_{4}'.format( args.model, args.data, 
    ...
    logger.info('-------------------- Edits with Inverse Attack - Soft truth ----------------------')
    ...
    triples_to_add, decoy_triples, summary_dict, inverse_relation = get_additions(train_data, test_data, model)
    ...
    save_path = 'data/inv_add_1_{0}_{1}_{2}_{3}_{4}'.format( args.model, args.data, 
                                                               args.target_split, args.budget, args.rand_run)
    ...
    with open(os.path.join(save_path, 'stats.txt'), 'w') as f:
        ...
        f.write('----------------------Inverse Add 1 (Soft truth)------------------------------- \n')
