...
#         rel_choices = rels[np.where(rels!=test_r)]
        rel_choices = rels
        # o needs to be excluded because the target triple should not be added to training data
        ...
        rand_o = rng.choice(a=ent_choices_o, size = 1, replace=True)[0]
        rand_r2 = rng.choice(a=rel_choices, size = 1, replace=True)[0]
        rand_s = rng.choice(a=ent_choices_s, size = 1, replace=True)[0]
        
        cand = {0: [test_s, rand_r1, rand_o],
                1: [rand_s, rand_r2, test_o]}
        rnd_idx = rng.choice(a=[0,1], size=1, replace=False)[0]
        add_trip = cand[rnd_idx]
...
log_path = 'logs/attack_logs/rand_add_n_{0}_{1}_{2}_{3}_{4}'.format( args.model, args.data, 
...
save_path = 'data/rand_add_n_{0}_{1}_{2}_{3}_{4}'.format(args.model, args.data, 
                                                    args.target_split, args.budget, args.rand_run
                                                                )
