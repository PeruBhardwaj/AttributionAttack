...
def get_deletions(train_data, test_data, neighbours, model, attack_batch_size):
    ...
    for test_idx, test_trip in enumerate(test_data):
        ...
        for train_trip in nghbr_trip:
            ...
            cos_sim = F.cosine_similarity(test_grads, train_grads, dim=0) #default dim=1
            nghbr_cos_sim.append(cos_sim.item())  
...
if __name__ == '__main__':
    ...
    log_path = 'logs/attack_logs/cos_grad_del_{0}_{1}_{2}_{3}_{4}'.format( args.model, args.data, 
    ...
    save_path = 'data/cos_grad_del_{0}_{1}_{2}_{3}_{4}'.format( args.model, args.data, 
                                                               args.target_split, args.budget, args.rand_run)
